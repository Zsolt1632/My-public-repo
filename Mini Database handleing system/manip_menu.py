...
class Select(tk.Frame):
    ...
    def add_checkboxes(self):
        ...
        for column_name in self.column_names:
            ...
            aggregate_var = tk.StringVar(self)
            aggregate_var.set("None")  # Default value
            aggregate_combo = ttk.Combobox(self, textvariable=aggregate_var,
                                           values=["None", "avg", "min", "max", "count", "sum"])
            aggregate_combo.pack(pady=5, padx=5)

            aggregate_combo.column_name = column_name
            self.aggregate_combos.append(aggregate_combo)
        ...
        self.checkboxes.append(self.group_by_checkbox)
        self.submit_button.pack(pady=10, padx=10)
    ...
    def reset(self):
        ...
        for widget in self.winfo_children():
            if isinstance(widget, ttk.Combobox) and widget.master == self:
                widget.destroy()
    ...
    def insert(self):
        ...
        for combo in self.aggregate_combos:
            if combo.get() in ["avg", 'count', 'sum', 'min', 'max']:
                agg_function = combo.get()
                column_name = combo.column_name
                aggregate_columns.append(f"{agg_function}({column_name})")
        if aggregate_columns:
            text += ", " + ", ".join(aggregate_columns)
